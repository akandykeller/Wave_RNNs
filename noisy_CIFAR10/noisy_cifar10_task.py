...
model = model_select[args.model_type].coRNN(n_inp, args.n_hid, n_out, args.n_ch, args.act, 
                                            args.ksize, args.init, args.freeze_rnn, args.solo_init).to(device)
...
fname = f'result/cifar10_log_{args.model_type}_h{args.n_hid}_lr{args.lr}.txt'
...
def test(data_loader):
    ...
    with torch.no_grad():
        for images, labels in data_loader:
            ...
            output, _, decode = model(images, get_seq=False)
            pred = output.data.max(1, keepdim=True)[1]
            correct += pred.eq(labels.data.view_as(pred)).sum()
    accuracy = 100. * correct / len(data_loader.dataset)
    recon_loss = recon_loss / len(data_loader.dataset)

    return accuracy.item(), recon_loss
...
for epoch in range(args.epochs):
    ...
    for i, (images, labels) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
        ## Reshape images for sequence learning:
        images = torch.cat((images.permute(0,2,1,3).reshape(-1,32,96),rand_train),dim=1).permute(1,0,2)
        ...
        if i % 100 == 0:
            log('Train Loss:', loss)
            _, y_seq, _ = model(images, get_seq=True)
            utils.Plot_Vid(y_seq.detach().cpu()[:, 0])
            if torch.isnan(loss):
                exit()

            if args.model_type == 'wrnn':
                model.plot_weights()

    valid_acc, valid_recon = test(valid_loader)
    test_acc, test_recon = test(test_loader)
